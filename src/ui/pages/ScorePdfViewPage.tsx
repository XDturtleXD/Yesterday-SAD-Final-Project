...
export function ScorePdfViewPage() {
  ...
  if (!project || !score) {
    return (
      <Card className="p-6">
        <div className="text-sm font-semibold text-slate-900">{t('scoreEditor.pdfNotFound')}</div>
        <div className="mt-4">
  ...
  return (
    <Card className="p-6">
      ...
      <div className="mt-1 text-sm text-slate-600">{t('scoreEditor.pdfPreviewUnavailable')}</div>
      <div className="mt-4">
        <Button variant="secondary" onClick={() => navigate(`/projects/${project.id}?tab=pieces`)}>
          {t('scoreEditor.backToScores')}
        </Button>
      </div>
    </Card>
  )
